...
if __name__ == '__main__':
    ...
    parser.add_argument('--share_dim', type=int, default=512,
                        metavar='N', help='domain share vector dimension')
    parser.add_argument('--unshare_dim', type=int, default=128,
                        metavar='N', help='domain share distribution dimension')
    ...
    parser.add_argument('--combination', type=str, default='add',
                        help='which method to combine share with unshare [add or concat]')
    ...
    for epoch in range(start_epoch, tot_epoch):
        ...
        for it in range(updates_per_epoch):
            ...
            one_hot_class_ids = to_device(ori_one_hot(np_class_ids, class_num=201))
            one_hot_wrong_class_ids = to_device(ori_one_hot(np_wrong_class_ids, class_num=201))
            ...
            for key, _ in fake_images.items():
                ...
                if isinstance(real_img_logit_local, tuple):
                    ...
                    real_labels, fake_labels = get_labels(logit)
                else:
            ...
            for key, _ in fake_images.items():
                ...
                if key == 'output_128':
                    fake_pair_logit, fake_img_logit_local, fake_cls_logit = netD(this_fake, embeddings)
                else:
